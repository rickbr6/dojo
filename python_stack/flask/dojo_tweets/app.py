...
def show_dashboard():
    ...
    if request.method == "GET":
        ...
        # Get tweets:
        mysql = connectToMySQL(SCHEMA)
        query = "SELECT tweets.id, CONCAT_WS(' ', users.first_name, users.last_name) as full_name, users.created_at, users.updated_at," \
...
def post_a_message():
    ...
    mysql = connectToMySQL(SCHEMA)
    query = "INSERT INTO tweets (user_id, content, created_at, updated_at) VALUES (%(uid)s, %(message)s, NOW(), NOW());"
    data = {
        'uid': user_id,
        'message': message
    }
